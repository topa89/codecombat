...
module.exports = class User extends CocoModel
  ...
  fetchGPlusUser: (gplusID, gplusAccessToken, options={}) ->
    ...
    options.data.gplusAccessToken = gplusAccessToken
    @fetch(options)

  fetchFacebookUser: (facebookID, facebookAccessToken, options={}) ->
    options.data ?= {}
    options.data.facebookID = facebookID
    options.data.facebookAccessToken = facebookAccessToken
    @fetch(options)
